...
- it employs an iterative approach known as **level-wise search**, where k-itemsets are used to explore (k+1)-itemsets, starting by k=1, there are 2 steps involved in each iteration:
	...
	- prune: $C_k$ could be the next set of k-itemsets in the iteration, but it would be so huge, thanks to the **apriori property**, we know that if we find non-frequent itemset, then all its supersets will be non-frequent, we could make use of that fact by filtering out itemsets in $C_k$, that doesn't  the result will be $L_k$ ready for the next iteration!
