...
- a Finite Automata is defined as a 5-tuple: $M = (Q, \Sigma, \delta, q_0, F)$, where:
	...
	- $\delta: Q \times \Sigma \rightarrow Q$ is a transition function; a mapping that defines what is the new state $q_{new} \in Q$ resulting from current state $q_{current} \in Q$ with an input $a \in \Sigma$ applied to it (ex, $\delta(q_1, A) = q_3, w$).