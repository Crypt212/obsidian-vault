...
marking.
- Transitions:
	1. Start: `0 1 \sqcup` head at `0`, state $q_0$​.  
	    $\delta(q_0,0) \rightarrow (q_1, X, R)$: tape `X 1 \sqcup`, state $q_1$, head on `1`.
	2. $\delta(q_1,1) \rightarrow (q_2, X, L)$: tape `X X \sqcup`, state $q_2$, head on `X` (first).
	3. $\delta(q_2,X) \rightarrow (q_2, X, L)$: move left to `\sqcup` left of first symbol.
	...
	9. $\delta(q_3,X) \rightarrow (q_3, X, R)$: head on `\sqcup`.
	10. $\delta(q_3,\sqcup) \rightarrow (q_{accept}, \sqcup, R)$: **accept**.- Start state: $q_0$
