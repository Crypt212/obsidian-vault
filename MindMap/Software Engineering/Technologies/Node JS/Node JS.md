...
- `exports`: the exports for which holds the exported data from the current module.
- `module`: is a `Module` instance that holds information for the current module, one example is the `exports` property (which is a reference to hhe `exports` parameter from the IIFE function).
## Module Caching
