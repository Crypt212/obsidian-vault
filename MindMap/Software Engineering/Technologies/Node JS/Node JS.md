...
- `module`: is a `Module` instance that holds information for the current module, one example is the `exports` property (differs from the `exports` parameter from the IIFE function) which holds the exported data from the current module.
## Module Caching
