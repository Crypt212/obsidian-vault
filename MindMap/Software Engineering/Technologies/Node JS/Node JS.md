...
- It is a module for managing paths, it has these methods:
	...
	- `resolve()`: works like `join()` with some differences, it always returns an absolute path, here is how:
		- if some of string arguments contain a separator at the beginning, it will take the last argument as the root ignore what is before it.
		- if non of the string arguments contain a separator, at the beginning it will prepend the `__dirname` to the resolved path.
	- 
- `events`
- `fs`
- `stream`
